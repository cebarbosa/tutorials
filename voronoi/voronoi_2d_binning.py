...
def voronoi_2d_binning(x, y, signal, noise, targetSN, cvt=True,
    ...
    assert x.size == y.size == signal.size == noise.size, 'Input vectors (x, y, signal, noise) must have the same size'
    assert np.all((noise >= 0) & np.isfinite(noise)), 'NOISE cannot be negative or NaN'
